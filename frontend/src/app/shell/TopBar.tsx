 ...
 *
 * На телефоне она сжимается до имени раздела и переключателя темы: место на экране 390 px
 * стоит дороже, чем удобство разработчика.
 *
...
export function TopBar({ device }: { device: Device }) {
  ...
  return (
    <header className="sticky top-0 z-30 border-b border-line bg-card/95 backdrop-blur">
      {/* Тонкая полоса «космоса»: отделяет служебную строку от данных и служит меткой
          загрузки при обновлении опросом. */}
      <div className="h-0.5 w-full bg-gradient-to-r from-space-deep via-space-glow to-space-mid" />
