...
# Здесь стояла `record_access` — единственное исключение из правила «журнал пишется сам».
# Она записывала выдачу ссылки на файл проекта с грифом: сессия такого события не видит,
# потому что оно ничего не меняет. Гриф снят
# ([ADR-0024](../../../docs/adr/ADR-0024-share-externally.md)), писателя у неё не осталось,
# и функция убрана вместе с ним. Правило «журнал пишется сам» снова без исключений, и
# возвращать его стоит только вместе с событием, которого сессия действительно не видит.
    ...
    rows = [
        {
            ...
            "changes": (
                mask_changes(entry.changes) if entry.target.audit_hides_values else entry.changes
            ),
