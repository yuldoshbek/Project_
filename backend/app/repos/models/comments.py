...
class Comment(Auditable, UUIDPrimaryKey, Timestamps, Base):
    ...
    def audit_is_classified(self) -> bool:
        ...
        Не потому, что он всегда закрытый, а потому, что журналу он не нужен: «кто и
        когда» журнал записывает, а «что написано» лежит в самой реплике и никуда не
        девается — удаление мягкое. Второе место хранения того же текста означало бы, что
        комментарий к проекту с грифом расползается по таблицам, из которых его уже не
        вычистить (ADR-0007), — и ради чего.
        """
        return True
