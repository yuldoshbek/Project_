...
class Document(Auditable, UUIDPrimaryKey, Timestamps, Base):
    ...
    def audit_hides_values(self) -> bool:
        ...
        никуда не девается — удаление мягкое. Вторая копия оказалась бы в таблице, из
        которой её уже не вычистить: журнал только на дозапись, неизменяемость держит
        триггер базы ([ADR-0010](../../../docs/adr/ADR-0010-audit-log.md)), а сам он
        переживает и удаление проекта, и очистку хранилища.

        Есть и техническая причина, по которой «всегда», а не «иногда»: свойство читается
        в момент сохранения, а владелец записи лежит в другой таблице — спросить «чей это
        проект» означало бы запрос к базе из обработчика сохранения.
        """
        return True
