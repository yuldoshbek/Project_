...
class Auditable:
    ...
    def audit_is_classified(self) -> bool:
        """Содержимое закрыто и в журнал не выдаётся (ADR-0007).

        По умолчанию `False`: у большинства записей грифа нет. Проект отвечает здесь по
        своему полю `classification` — свойство, а не поле, потому что гриф может
        меняться, и журнал обязан смотреть на текущее значение, а не на снимок.
        """
        return False
